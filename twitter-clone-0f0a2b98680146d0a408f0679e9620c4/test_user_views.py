...
class MessageViewTestCase(TestCase):
    ...
    def test_users_index(self):
        with app.app_context():
            with self.client as c:
                # Response of users route
                resp = c.get("/users")  
                # Users should be in the response data
    ...
    def test_users_search(self):
        with app.app_context():
            with self.client as c:
                ...
                # Things with "test" in response data
                self.assertIn("@testuser", str(resp.data))
                self.assertIn("@testing", str(resp.data))            
    ...
    def test_user_show(self):
        with app.app_context():
            with self.client as c:
                # Response of showing user
                resp = c.get(f"/users/{self.testuser_id}")

                self.assertEqual(resp.status_code, 200)
                # User is in response data
                self.assertIn("@testuser", str(resp.data))
    ...
    def test_user_show_with_likes(self):
        with app.app_context():
            ...
            with self.client as c:
                ...
                # Getting response of user show
                resp = c.get(f"/users/{self.testuser_id}")

                # Testing accurate response
                self.assertEqual(resp.status_code, 200)
                self.assertIn("@testuser", str(resp.data))

                # Estracting HTML from response
                soup = BeautifulSoup(str(resp.data), 'html.parser')
                # Picking specific stuff from HTML
                found = soup.find_all("li", {"class": "stat"})

                # Should be 4 instances of lis with {"class": "stat"}, 4 categories 
    ...
    def test_add_like(self):
        with app.app_context():
            ...
            with self.client as c:
                ...
                # Response of liking something 
                resp = c.post("/messages/add_like/1984", follow_redirects=True)
                self.assertEqual(resp.status_code, 200)

                # Finding likes and checking content 
    ...
    def test_unauthenticated_like(self):
        with app.app_context():
            ...
            with self.client as c:
                # Liking a post while not logged in, not authorized 
                resp = c.post(f"/messages/add_like/{m.id}", follow_redirects=True)
                self.assertEqual(resp.status_code, 200)
    ...
    def test_user_show_with_follows(self):
        with app.app_context():
            ...
            with self.client as c:
                ...
                self.assertEqual(resp.status_code, 200)
                self.assertIn("@testuser", str(resp.data))
                # Finding all categories in HTML
                soup = BeautifulSoup(str(resp.data), 'html.parser')
                found = soup.find_all("li", {"class": "stat"})
    ...
    def test_show_following(self):
        with app.app_context():
            ...
            with self.client as c:
                ...
                # Getting following response and checking accuracy 
    ...
    def test_unauthorized_following_page_access(self):
        with app.app_context():
            self.setup_followers()
            with self.client as c:
                # Following pages but not logged in
    ...
    def test_unauthorized_followers_page_access(self):
        with app.app_context():
            self.setup_followers()
            with self.client as c:
                # Followers pages but not logged in
